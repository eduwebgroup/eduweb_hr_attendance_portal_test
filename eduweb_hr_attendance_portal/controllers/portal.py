...
class CustomerAttendancePortal(CustomerPortal):
    ...
    def _prepare_portal_layout_values(self):
        values = super(CustomerAttendancePortal, self)._prepare_portal_layout_values()
        employee_ids = request.env['hr.employee'].sudo().search([("user_id","=",request.env.uid)]).ids
        values["attendance_count"] = request.env["hr.attendance"].sudo().search_count([("employee_id","in",employee_ids)])
    ...
    def portal_my_attendances(self, page=1, date_begin=None, date_end=None, sortby=None, search=None, search_in="check_in", **kw):
        ...
        return request.render("eduweb_hr_attendance_portal.portal_my_attendances", values)
    ...
    def portal_my_attendance(self, attendance_id=None, access_token=None, **kw):
        ...
        return request.render("eduweb_hr_attendance_portal.portal_my_attendance", values)